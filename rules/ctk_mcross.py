...
rule all:
    input:
        ...
        mdtag = f"{OUTPUT}/bams/genome/{replicate_label}.genome.Aligned.sort.dedup.umap.md.sam",
...
rule select_informative_read:
    ...
    output:
        bam_informative=f"{OUTPUT}/bams/genome_R1/{replicate_label}.genome.Aligned.sort.dedup.R1.bam"
    params:
        error_out_file = f"{OUTPUT}/stderr/{replicate_label}.select_informative_read.err",
...
rule md_tag:
    ...
    params:
        fa = "/tscc/projects/ps-yeolab4/genomes/GRCh38/chromosomes/GRCh38_no_alt_analysis_set_GCA_000001405.15.fasta",
        error_out_file = f"{OUTPUT}/error_files/md_tag.{replicate_label}.err",
        out_file = f"{OUTPUT}/stdout/md_tag.{replicate_label}.out",
        run_time = "1:00:00",
        memory = 5000,
        cores = 1,
        job_name = "fillmd",
...
rule ctk_parse:
    ...
    output:
        ...
        peak_PH = f"{OUTPUT}/ctk/{replicate_label}.uniq.peak.sig.halfPH.bed",
    params:
        error_out_file = f"{OUTPUT}/error_files/ctk_parse.{replicate_label}.err",
        out_file = f"{OUTPUT}/stdout/ctk_parse.{replicate_label}.out",
        run_time = "2:00:00",
        memory = 10000,
        cores = 1,
        job_name = "parse",
...
rule ctk_CIMS:
    ...
    shell:
        """
        perl /tscc/nfs/home/s5xu/projects/ctk-1.1.5/CIMS.pl -big -n 10 -p -outp {output.mutation_CIMS_stat} -v {input.tagbed} {input.mutation} {output.mutation_CIMS}
        """
...
rule fetch_CIMS_sequence:
    ...
    params:
        error_file = os.path.join(OUTPUT, "stderr/CIMS.{replicate_labels}.{mutation_types}.fetch_sequence.err"),
...
rule ctk_CITS:
    ...
    shell:
        """
        perl /tscc/nfs/home/s5xu/projects/ctk-1.1.5/CITS.pl -big -p 0.001 --gap 25 -v {input.tagbed} {input.mutation} {output.mutation_CITS}
        """   
...
rule fetch_CITS_sequence:
    ...
    params:
        error_file = os.path.join(OUTPUT, "stderr/CITS.{replicate_labels}.{mutation_types}.fetch_sequence.err"),
...
rule mcross_get_kmer_seed:
    ...
    params:
        error_file = os.path.join(OUTPUT, "error_files/{data_types}.{replicate_labels}.{mutation_types}.mcross_kmer.err"),
