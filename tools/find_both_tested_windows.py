...
if __name__ == '__main__':
    ...
    # if NaNs present in either tested window, these fields become floats and things break
    both_tested_windows['start'] = both_tested_windows['start'].astype(int)
    both_tested_windows['end'] = both_tested_windows['end'].astype(int)
    assert both_tested_windows[both_tested_windows.isna().all(axis=1)].shape[0] == 0  # assert NaNs dont exist in both windows
