...
def gc_fraction(gc_bin_sum, exp, ip_rep, in_rep):
    ...
    return gc_frac
def get_gc_odds_ratio_across_rep(gc_bin_sum, exp, lib = 'IP'):
    gc_odds = (gc_bin_sum[f'{exp}_{lib}_1']/gc_bin_sum[f'{exp}_{lib}_2']
          )/(total[f'{exp}_{lib}_1']/total[f'{exp}_{lib}_2'])
...
if __name__ == '__main__':
    ...
    ip_rep_bias = get_gc_odds_ratio_across_rep(gc_bin_sum, exp)
    bias_data['ip_l2odds_diff'] = np.log2(ip_rep_bias).abs().sum()

    if in_reps == [1,2]:
        in_rep_bias = get_gc_odds_ratio_across_rep(gc_bin_sum, exp, lib = 'IN')
        bias_data['in_l2odds_diff'] = np.log2(in_rep_bias).abs().sum()
    
    pd.Series(bias_data,name = exp).to_csv(outf)