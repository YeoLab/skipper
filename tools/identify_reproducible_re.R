...
if (nrow(enriched_re_data) > 0) {
    reproducible_enriched_re_data = enriched_re_data %>%
        group_by(across(-c(clip_replicate_label, gc_bin, baseline_l2or, input,
                           clip, enrichment_l2or, pvalue, qvalue))) %>%
        summarize(
            input_sum = sum(input, na.rm = TRUE),             
            clip_sum = sum(clip, na.rm = TRUE),              
            enrichment_n = sum(qvalue < 0.05, na.rm = TRUE),   
        ...
        mutate(
            across(c(enrichment_l2or_min, enrichment_l2or_mean, enrichment_l2or_max, p_max, p_min, 
                     q_max, q_min),
                   ~ ifelse(is.infinite(.), NA_real_, .))
        ) %>%
    	filter(enrichment_n > (length(enriched_re_files) - 1)) %>%                
        arrange(p_min, desc(enrichment_l2or_mean))
...
if (nrow(reproducible_enriched_re_data) < 1) {
    ...
    base_group_cols = setdiff(names(enriched_re_data), c("clip_replicate_label", "gc_bin", "baseline_l2or", "input",
                                                         "clip", "enrichment_l2or", "pvalue","qvalue"))
    ...
    if (nrow(enriched_re_data) > 0 && length(base_group_cols) > 0) {
        ...
        # Full fallback if there is truly no structure to borrow.
        reproducible_enriched_re_data = tibble(
            input_sum = NA_real_,
            clip_sum = NA_real_,
            enrichment_n = NA_integer_,
    ...
    plot.new()
    text(0.5, 0.5, labels = "No reproducible enriched repeat elements.\nDummy output generated.", cex = 0.7)
    dev.off()
}
